...
import org.nlogo.window.SwingUnlockedExecutionContext
...
with ModelSavedEvent.Handler {
  ...
  locally {
    ...
    new OpenModelFromURI(SwingUnlockedExecutionContext)(Paths.get(filePath).toUri, controller, loader, fileformat.defaultConverter, Version).foreach { model =>
